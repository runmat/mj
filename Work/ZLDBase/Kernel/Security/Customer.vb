...
#Region " Membervariables "
        ...
        Private m_mvcLayout As String
        Private m_message As String
        Private m_maxReadMessageCount As Integer
...
#Region " Constructor "
        ...
        Public Sub New(ByVal intCustomerID As Integer, _
                               ...
                               ByVal intMaxReadMessageCount As Integer, _
                               ...
                               Optional ByRef strSDSignatur As String = "")
            ...
            m_maxReadMessageCount = intMaxReadMessageCount
            m_DcDatCredentials = New DatCredentials(strSDCustomerNumber, strSDUserName, strSDPassword, strSDUserLogin, strSDSignatur)
        End Sub
...
#Region " Properties "
        ...
        Public ReadOnly Property MvcLayoutAsWebFormsInline() As Boolean
            Get
                Return (Not String.IsNullOrEmpty(m_mvcLayout)) And (m_mvcLayout.ToLower() = "services")
            End Get
        End Property
        ...
        Public Property MaxReadMessageCount As Integer
            Get
                Return m_maxReadMessageCount
            End Get
            Set(value As Integer)
                m_maxReadMessageCount = value
            End Set
        End Property
...
#Region " Functions "
        ...
        Private Sub GetCustomer(ByVal cn As SqlClient.SqlConnection)
            ...
            Try
                While dr.Read
                    ...
                    Try
                        m_mvcLayout = dr("MvcLayout").ToString
                    Catch
                        m_mvcLayout = ""
                    End Try
                    ...
                    If Not TypeOf dr("MaxReadMessageCount") Is System.DBNull Then
                        m_maxReadMessageCount = CInt(dr("MaxReadMessageCount"))
                    Else
                        m_maxReadMessageCount = 0
                    End If
                End While
            Catch ex As Exception
                Throw ex
        ...
        Public Sub GetSDCredentials(ByVal cn As SqlClient.SqlConnection)
            Dim cmdSDCredentials As New SqlClient.SqlCommand("SELECT Kundennummer, Zugangsname, Passwort, LoginNameBankenLinie, SignaturBankenLinie FROM CustomerDAT WHERE CustomerID=@CustomerID", cn)
            ...
            Using dr As SqlClient.SqlDataReader = cmdSDCredentials.ExecuteReader()
                If dr.HasRows Then
                    dr.Read()
                    m_DcDatCredentials = New DatCredentials(dr.GetString(0), dr.GetString(1), dr.GetString(2), dr.GetString(3), dr.GetString(4))
                Else
        ...
        Public Sub Save(ByVal cn As SqlClient.SqlConnection)
            Try
                ...
                With cmd.Parameters
                    ...
                    .AddWithValue("@Message", m_message)
                    .AddWithValue("@MaxReadMessageCount", m_maxReadMessageCount)
        ...
        Private Sub SaveSDCredentials(ByVal cn As SqlClient.SqlConnection)
            ...
            If Convert.ToInt32(cmdSDCredentials.ExecuteScalar()) = 0 Then
                cmdSDCredentials.CommandText = "INSERT INTO CustomerDAT(CustomerID, Kundennummer, Zugangsname, Passwort, LoginNameBankenLinie, SignaturBankenLinie) " & _
                                               "VALUES(@CustomerID, @Kundennummer, @Zugangsname, @Passwort, @LoginNameBankenLinie, @SignaturBankenLinie)"
            Else
...
#Region " Constructor "
            Friend Sub New(ByVal strCustomerNumber As String, _
                           ...
                           ByRef strPassword As String, ByRef strLoginName As String, ByRef strSignatur As String)
