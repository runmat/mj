...
#Region "Methoden"
        ...
        Public Shared Function FormatGridDateColumn(ByVal alDateColums As ArrayList, ByVal dataSource As DataTable) As DataTable
            Dim dr As DataRow
            Dim tmpColumn As New DataColumn()
            ...
            For Each tmpColumn In dataSource.Columns
                If alDateColums.Contains(tmpColumn.ColumnName.ToString().ToUpper) = True Then
                    tmpTable.Columns.Add(tmpColumn.ColumnName, System.Type.GetType("System.DateTime"))
        ...
        ''' <returns>Das Ergebniss der Prüfung</returns>
        Public Overloads Shared Function checkDate(ByRef date1 As System.Web.UI.WebControls.TextBox, ByRef date2 As System.Web.UI.WebControls.TextBox, ByRef errorText As String, ByVal allowEmpty As Boolean, Optional ByVal maxDateDiffInYears As Int32 = 0) As Boolean

            If date1.Text = "" And date2.Text = "" And allowEmpty = False Or IsDate(date1.Text) = False And date1.Text = "" = False Or IsDate(date2.Text) = False And date2.Text = "" = False Then

                If IsDate(date1.Text) = False And Not date1.Text = "" And date2.Text = "" = True Or IsDate(date2.Text) = True Then
                    errorText = "Geben Sie bitte ein gültiges Datum ein: " & date1.ToolTip
                    checkDate = False
                    Exit Function
                End If

                If IsDate(date2.Text) = False And Not date2.Text = "" And date1.Text = "" = True Or IsDate(date1.Text) = True Then
            ...
            For Each tmpRow As DataRow In datentabelle.Rows
                For i As Int32 = 0 To tmpRow.ItemArray.Length - 1
                    If tmpRow(i) Is DBNull.Value Then
                        If tmpRow.Table.Columns(i).DataType Is Type.GetType("System.DateTime") Then
                            Dim dat As DateTime
                            DateTime.TryParse("00.00.0000", dat)
                            tmpRow(i) = dat
                        Else
        ...
        Public Shared Sub fakeLogEntryForAutorisierung(ByVal logAppObj As Base.Kernel.Logging.Trace, ByVal m_User As CKG.Base.Kernel.Security.User, ByVal sessionID As String, _
            ...
            If Not logAppObj.WriteEntry(strType, m_User.UserName, sessionID, appIDofAutorisation, "Autorisierung", AppName, strMessage, m_User.CustomerName, m_User.Customer.CustomerId, m_User.IsTestUser) Then
                Throw New Exception("Fehler beim der WriteEntry Funktion in HelpProcedures.fakeLogEntryForAutorisierung")
            End If
