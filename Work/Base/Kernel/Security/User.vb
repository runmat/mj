...
#Region " Functions "
        ...
        Public Sub SaveQuestion(ByVal inQuestionID As Integer, ByVal strQuestionText As String)
            ...
            Catch ex As Exception
                Throw New Exception("Fehler beim Ändern des Kennwortes: " & ex.Message, ex)
            Finally
                If cn.State <> ConnectionState.Closed Then
                    cn.Close()
                End If
                ...
                If m_intUserId = -1 Then
                    If GetUserDataFromName(strUsername, cn) Then
                        ...
                        Else
                            ...
                            End If
                            Throw New System.Exception("Das Kennwort wurde mehrfach falsch eingegeben. Das Benutzerkonto wurde gesperrt. Bitte setzen Sie sich mit Ihrem Administrator in Verbindung!")
                    End If
                    Else
                        Throw New System.Exception("")
                    End If
        ...
        Public Function Login(ByVal strUsername As String, ByVal strPassword As String, _
            ...
            Try
                ...
                If Not (Len(strUsername) > 0 And Len(strPassword) > 0) Then
                    Throw New System.Exception("Keine gültigen Anmeldedaten eingegeben!")
                End If
                'Kennwort mit Leerzeichen
                If strPassword.Length <> strPassword.Trim.Length Then
                    Throw New System.Exception("Überprüfen Sie Ihr Kennwort! Leerzeichen nicht erlaubt!")
                End If
        ...
        Private Function VerifyPassword(ByVal strPassword As String, ByVal cn As SqlClient.SqlConnection) As Boolean
            Try
                ...
                Return False
            Catch ex As Exception
                Throw New Exception("Fehler bei der Kennwortüberprüfung!", ex)
            End Try
        End Function
        ...
        Private Sub SavePasswordHistory(ByVal strPassword As String, ByVal LastPswdChange As Date, ByVal cn As SqlClient.SqlConnection, Optional ByVal blnInitialPswd As Boolean = False)
            Try
                ...
                End If
            Catch ex As Exception
                Throw New Exception("Fehler beim Schreiben der Kennworthistorie!", ex)
            End Try
        End Sub
                            ...
                            If strNewPwd = strNewPwdConfirm Then
                                If m_customer.CustomerPasswordRules.PasswordIsValid(strNewPwd) Then
                                    If CheckPasswordHistory(strNewPwd, cn) Then
                                        ...
                                        End Try
                                    Else
                                        m_strErrorMessage = String.Format("Dieses Kennwort kann nicht wiederverwendet werden. Die letzten {0} Kennwörter eines Benutzers sind für eine nochmalige Verwendung gesperrt.", m_customer.CustomerPasswordRules.PasswordHistoryEntries)
                                    End If
                                Else
                                    m_strErrorMessage = m_customer.CustomerPasswordRules.ErrorMessage
                                End If
                            Else
                                m_strErrorMessage = "Das neue Kennwort und die Kennwortbestätigung müssen einander entsprechen."
                            End If
                        Else
                            m_strErrorMessage = "Das alte und das neue Kennwort dürfen nicht gleich sein."
                        End If
                    Else
                        m_strErrorMessage = "Leere Zeichenfolge ist nicht erlaubt!"
                    End If
                Else
                    m_strErrorMessage = "Sie müssen Ihr altes Kennwort angeben!"
                End If
            ElseIf strOldPwd.Length <> strOldPwd.Trim.Length Then
                m_strErrorMessage = "Überprüfen Sie Ihr altes Kennwort! Leerzeichen nicht erlaubt!"
            Else
                m_strErrorMessage = ""
            End If
            Return False
                ...
                If (Not m_customer.CustomerPasswordRules.DontSendEmail) Or blnSendPasswordAnyway Then
                    If (m_mail <> String.Empty) Then
                        ...
                        With textBuilder
                            ...
                            .Append(ControlChars.CrLf)
                            .Append("Ihr persönliches Kennwort für den Internetbereich der Kroschke Gruppe und dem Deutschen Auto Dienst lautet:")
                            .Append(ControlChars.CrLf)
                            .Append(ControlChars.CrLf)
                            .Append(password & " (Bitte achten Sie auf die korrekte Groß- und Kleinschreibung)")
                            .Append(ControlChars.CrLf)
                            .Append(ControlChars.CrLf)
                            .Append("Mit diesem Kennwort sowie Ihrem Usernamen können Sie nun das Login vornehmen.")
                            .Append(ControlChars.CrLf)
                            .Append("Wir müssen Sie bitten, Ihr Kennwort nach der ersten Anmeldung aus Sicherheitsgründen zu ändern.")
                            .Append(ControlChars.CrLf)
                            .Append("Bitte bewahren Sie Ihren Usernamen sowie Ihr Online-Kennwort an einem sicheren Platz auf und geben Sie Ihre Zugangsdaten nicht an Dritte weiter.")
                            .Append(ControlChars.CrLf)
                            .Append(ControlChars.CrLf)
                    ...
                    If (m_mail <> String.Empty) Then
                        ...
                        If Reapproved Then
                            With textBuilder
                                ...
                                .Append(ControlChars.CrLf)
                                .Append(ControlChars.CrLf)
                                .Append("Ihr persönlicher Username für den Internetbereich der Kroschke Gruppe und dem Deutschen Auto Dienst wurde wieder entsperrt.")
                                .Append(ControlChars.CrLf)
                                .Append(ControlChars.CrLf)
                                .Append("Ihr Username lautet: " & m_strUsername)
                                .Append(ControlChars.CrLf)
                                .Append(ControlChars.CrLf)
                                .Append("Mit diesem Username sowie Ihrem persönlichen Kennwort können Sie das Login vornehmen.")
                                .Append(ControlChars.CrLf)
                                .Append(ControlChars.CrLf)
                                .Append("Hinweis!: Das persönliche Kennwort erhalten Sie in einer separaten E-mail.")
                                .Append(ControlChars.CrLf)
                                .Append(ControlChars.CrLf)
                                .Append("Mit freundlichen Grüßen")
                                .Append(ControlChars.CrLf)
                        ...
                        Else
                            With textBuilder
                                ...
                                .Append(ControlChars.CrLf)
                                .Append(ControlChars.CrLf)
                                .Append("Ihr persönlicher Username für den Internetbereich der Kroschke Gruppe und dem Deutschen Auto Dienst lautet:")
                                .Append(ControlChars.CrLf)
                                .Append(ControlChars.CrLf)
