...
Namespace Kernel.Common
    Public Class Common
        ...
        Public Shared Sub SetEndASPXAccess(ByVal Form As System.Web.UI.Page)
            'Seite übersetzen
            Dim AppURL As String
            Try
                AppURL = Replace(Form.Request.Url.LocalPath, "/" & m_strAppKey & "", "..")
            Catch
                AppURL = ""
            End Try
            If AppURL.Length > 0 Then
                If Not Form.Session(AppURL) Is Nothing Then
                    Dim tblTranslations As New DataTable()
                    tblTranslations = CType(Form.Session(AppURL), DataTable)
                    If Not tblTranslations Is Nothing AndAlso tblTranslations.Rows.Count > 0 Then
                        Dim outerControl As System.Web.UI.Control
                        For Each outerControl In Form.Controls
            ...
            If (Not Form.Session("log") Is Nothing) And Not (Form.Session("objUser") Is Nothing) Then
                'Dim _user As User
