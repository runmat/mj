...
Namespace Common
    ...
    Public Class DynSapProxyErp
        Inherits DynSapProxyObjErp
...
#Region "Methods"
        ...
        ''' </summary>
        ''' <returns></returns>
        Public Shared Function getProxy(ByVal bapiName As String, ByRef mObjApp As CKG.Base.Kernel.Security.App, ByRef mObjUser As CKG.Base.Kernel.Security.User, ByRef mObjPage As Web.UI.Page) As DynSapProxyObjErp
            ...
            If Not bapiName.Trim(" "c) = "" AndAlso Not mObjPage Is Nothing AndAlso Not mObjApp Is Nothing AndAlso Not mObjUser Is Nothing Then
                If Not checkBapiInDB(bapiName.ToUpper, blnTestSap) Then
                    Dim tmpProxyObj As DynSapProxyObjErp = generateNewProxy(bapiName.ToUpper)
                    ...
                    End If
                End If
                Dim tmpObj As DynSapProxyObjErp
        ...
        Protected Function getDirektSapStruktur(ByVal BapiName As String) As DynSapProxyObjErp
            Return generateNewProxy(BapiName)
        End Function
        ...
        Private Shared Function checkBapiInDB(ByVal bapiName As String, ByVal testSap As Boolean) As Boolean
            ...
            Try
                ...
                cmd.CommandText = "Select * FROM BapiStruktur WHERE BapiName=@BapiName AND ISNULL(TestSap,0)=@TestSap AND SourceModule=@SourceModule;"
                cmd.Parameters.AddWithValue("@BapiName", bapiName)
                cmd.Parameters.AddWithValue("@TestSap", testSap)
        ...
        Private Shared Function CallbapiForBapi(ByRef mExportTabelle As DataTable, ByRef mImportTabelle As DataTable, ByVal bapiName As String) As Date
            ...
            Try
                ...
                Dim func As RFCFunction = con.CreateFunction("Z_S_GET_BAPI_INTERFACE")
        ...
        Private Shared Function generateNewProxy(ByVal bapiName As String) As DynSapProxyObjErp
            '----------------------------------------------------------------------
            ...
            Try
                ...
                For Each tmprowX As DataRow In expTable.Rows
                    'CHC Fehler beim umwandeln 20121025
                ...
                Return New DynSapProxyObjErp(bapiName, BapiSapDate, ImportStruktur, ExportStruktur)
            Catch ex As Exception
        ...
        Private Shared Function getBapiStrukturFromSQLDB(ByVal BapiName As String, ByVal testSap As Boolean) As DynSapProxyObjErp
            '----------------------------------------------------------------------
            ...
            Try
                cn.Open()
                Dim cmd As New SqlCommand("SELECT * FROM BapiStruktur WHERE BapiName=@BapiName AND ISNULL(TestSap,0)=@TestSap AND SourceModule=@SourceModule", cn)
                cmd.Parameters.AddWithValue("@BapiName", BapiName)
                cmd.Parameters.AddWithValue("@TestSap", testSap)
                cmd.Parameters.AddWithValue("@SourceModule", SourceModuleString)
                ...
                If Not myDataReader Is Nothing AndAlso myDataReader.Read Then
                    Return New DynSapProxyObjErp(BapiName, CDate(myDataReader.GetValue(1)), CType(myFormatter.Deserialize(New MemoryStream(DirectCast(myDataReader.GetValue(2), Byte()))), DataTable), DirectCast(myFormatter.Deserialize(New MemoryStream(DirectCast(myDataReader.GetValue(3), Byte()))), DataTable))
                Else
                    Return Nothing
                End If
            Catch ex As Exception
        ...
        Private Shared Sub writeStrukturIntoSQLDB(ByVal ImportStruktur As DataTable, ByVal ExportStruktur As DataTable, ByVal bapiDate As Date, ByVal bapiname As String, ByVal testSap As Boolean)
            ...
            Try
                ...
                If Not checkBapiInDB(bapiname, testSap) Then
                    cmd.CommandText = "INSERT INTO [BapiStruktur] (BapiName, BapiDate, TestSap, SourceModule, ImportStruktur, ExportStruktur) VALUES (@BapiName, @BapiDate, @TestSap, @SourceModule, @ImportStruktur, @ExportStruktur);"
                    With cmd
                    ...
                    End With
                Else
                    cmd.CommandText = "UPDATE BapiStruktur SET ImportStruktur=@ImportStruktur, ExportStruktur=@ExportStruktur, BapiDate=@BapiDate, updated=@updated WHERE BapiName=@BapiName AND ISNULL(TestSap,0)=@TestSap AND SourceModule=@SourceModule;"
                    With cmd
                        .Parameters.AddWithValue("@ImportStruktur", serializedI)
                        .Parameters.AddWithValue("@ExportStruktur", serializedE)
                        .Parameters.AddWithValue("@BapiDate", bapiDate)
