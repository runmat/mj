...
#Region " Constructor "
        ...
        Public Sub New(ByVal intCustomerID As Integer, ByVal cn As SqlClient.SqlConnection)
            '06/03/12 auskommentiert da keine Abfrage stattfindet
            '----------------------------------------------------
            'Try
        ...
        Public Sub New(ByVal strAppNameFilter As String, ByVal cn As SqlClient.SqlConnection, ByVal strAppFriendlyNameFilter As String, Optional ByVal strAppTechType As String = "")
            Try
                ...
                Dim strCommand As String = "select a.*, p.AppName as AppParentName from Application a " & _
                                                "left outer join application p on a.AppParent=p.AppID " & _
                                                "where a.AppName like @AppName " & _
                                                "and a.AppFriendlyName like @AppFriendlyName"
                If Not String.IsNullOrEmpty(strAppTechType) Then
                    strCommand &= " and a.AppTechType = @AppTechType"
                End If
                Dim daApp As New SqlDataAdapter(strCommand, cn)
                ...
                If Not String.IsNullOrEmpty(strAppTechType) Then
                    daApp.SelectCommand.Parameters.AddWithValue("@AppTechType", strAppTechType)
                End If
                daApp.Fill(Me)
...
#Region " Functions "
        ...
        Public Sub GetUnassigned(Optional ByVal portalType As String = "")
            If Not m_blnTableFilled Then
                Dim daGroups As New SqlClient.SqlDataAdapter()
                daGroups.SelectCommand = New SqlClient.SqlCommand()
                daGroups.SelectCommand.Connection = m_cn
                ...
                ElseIf m_intGroupID = -2 Then
                    ...
                    Else
                        strSQL = "SELECT AppId, AppURL, AppName, AppFriendlyName, AppType, AppTechType, AppDescription, convert(bit, 0) AppIsMvcDefaultFavorite " & _
                                 ...
                                 "WHERE (CustomerID = @CustomerID) " & _
                                   "AND (AppParent=0)"
                        If Not String.IsNullOrEmpty(portalType) Then
                            Select Case portalType
                                Case "Portal", "Services"
                                    strSQL &= " AND (ISNULL(AppTechType,'') = '" & portalType & "' OR ISNULL(AppTechType,'') = 'MVC' OR ISNULL(AppTechType,'') = '')"
                                Case Else
                                    strSQL &= " AND (ISNULL(AppTechType,'') = '" & portalType & "' OR ISNULL(AppTechType,'') = '')"
                            End Select
                        End If
                    End If
                Else
