...
namespace StockCapture
{
    public class StockService
    {
        public static void CaptureStockQuote()
        {
            ...
            var price = myXmlDocument.SelectSingleNode("query/results/quote/LastTradePriceOnly", nsmgr).InnerText;
            ...
            var stockDateTime = DateTime.Parse(string.Format("{0} {1}", date, time), new CultureInfo("en-US"));
            stockDateTime = stockDateTime.AddHours(1);

            using (var repository = new Repository())
            {
                var sq = new StockQuote
                {
                    Val = double.Parse(price.Replace('.', ',')),
                    Date = stockDateTime,
                    InsertDate = DateTime.Now,
                };
                repository.SaveStockQuote(sq);

                var prevSqs = repository.RetrieveEquals<StockQuote>("ID", sq.ID - 1);
                if (prevSqs != null && prevSqs.Any())
                    sq.DiffToPrev = sq.Val - prevSqs.First().Val;
                var prevPrevSqs = repository.RetrieveEquals<StockQuote>("ID", sq.ID - 2);
                if (prevPrevSqs != null && prevPrevSqs.Any())
                    sq.DiffToPrevPrev = sq.Val - prevPrevSqs.First().Val;
        ...
        static string GetStockXml()
        {
            const string url = "http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20yahoo.finance.quotes%20where%20symbol%3D%22EURUSD%3DX%22&diagnostics=true&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys";
