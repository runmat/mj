...
namespace StockCapture
{
    public class StockService
    {
        ...
        public static void CaptureStockQuote(string stockName, out double price, out DateTime dateTime)
        {
        ...
        static string GetStockXml(string stockName)
        {
            var symbol = "EURUSD";
            if (stockName.ToLower().Contains("gold"))
                symbol = "XAUUSD";
            var url = string.Format("http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20yahoo.finance.quotes%20where%20symbol%3D%22{0}%3DX%22&diagnostics=true&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys", symbol);
