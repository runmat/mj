        ...
        protected void TransferPictures()
        {
            ...
            try
            {
                do
                {
                    ...
                    {
                        using (var bitmap = new BitmapRenderer(hbitmap).RenderToBitmap())
                        {
                            var args = new TransferImageEventArgs(bitmap, pendingTransfer.Count != 0);
                            TransferImage(this, args);

                            // Dispose!!!
                            //
                            // !! weiter unten machen (unter 'finally'), weil sonst nach dem 8. Scan ein native Speicher Fehler bei "Twain32Native.DsPendingTransfer"
                            //
                            //Marshal.FreeHGlobal(hbitmap);

                            // Dispose!!!
                            //
                            // !! wird im User Context gemacht (unter 'TransferImage')
                            //
                            //bitmap.Dispose();
