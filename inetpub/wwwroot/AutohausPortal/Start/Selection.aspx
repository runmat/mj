				...
				<div class="teasercontent" id ="teasercontent1" runat="server">
                    ...
                    <ul>
                        <% for (int i = 0; i < 4 && i < MenuChangeSource.Count; i++)
                            ...
                            <li>
                                <a href='<%= ResolveClientUrl(GetUrlString(row["AppURL"].ToString(), row["AppID"].ToString())) %>' onclick="LogPageVisit('<%= row["AppId"] %>', '<%= ResolveClientUrl(GetUrlString(row["AppURL"].ToString(), row["AppID"].ToString())) %>')"><%= row["AppFriendlyName"] %></a>
                            </li>
                        <% } %>
                    </ul>   
				</div>
