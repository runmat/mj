...
{
    /// <summary>
    /// Auftragseingabe für 72h-Versandzulassungen. Benutzte Klassen AHErfassung und objCommon.
    /// </summary>
    public partial class _72hZulassung : System.Web.UI.Page
    {
        private User m_User;
        private App m_App;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (IsPostBack != true)
            {
                ...
                {
                    objVorerf = new AHErfassung(ref m_User, m_App, "AV");
                    objVorerf.NrMaterial = "598";
                    objVorerf.Material = "72h-Versandzulassung";
                    fillForm();
                }
                Session["objVorerf"] = objVorerf;

                //form1.Attributes.Add("style", "display:block;");
            }
    ...
    "<script type='text/javascript'>disableDefaultValue('" + txtBox.ClientID + "');</script>", false);
        ...
        private void SelectValues()
        {
            ...
            //ddlStVa1.Items.Clear();
            //ddlStVa1.Items.Add(new RadComboBoxItem(objVorerf.Kreis, objVorerf.KreisKennz));
            ...
            {
                DataRow[] kennzRow = objCommon.tblKennzGroesse.Select("Groesse ='" + objVorerf.KennzForm + "' AND Matnr= '593'");
        ...
        private Boolean checkDate()
        {
            ...
            if (ZDat != String.Empty)
            {
                ...
                else
                {
                    ...
                    DateTime DateNew;
                    DateTime uebermorgen = tagesdatum.AddDays(2);
                    DateTime.TryParse(ZDat, out DateNew);
                    if (DateNew < uebermorgen)
                    {
                        divZulDate.Attributes["class"] = "formfeld error";
                        lblError.Text = "Bei einer Versandzulassung muss das Zulassungsdatum min. 2 Tage in der Zukunft liegen!";
                        bReturn = false;
                    }
                    else
                    {
