...
{
    /// <summary>
    /// Auftragseingabe für Umkennzeichnungen. Benutzte Klassen AHErfassung und objCommon.
    /// </summary>
    public partial class Umkennzeichnung : Page
    {
        private User m_User;
        private App m_App;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    objVorerf = new AHErfassung(ref m_User, m_App, "AU", "596");
                    fillForm();
        ...
        private void proofInserted()
        {
            ...
            if (txtAltKennz1.Text != "CK") { disableDefaultValue(txtAltKennz1); }
            if (txtAltKennz2.Text != "XX9999") { disableDefaultValue(txtAltKennz2); }
        ...
        private void fillForm()
        {
            ...
            if (objVorerf.IsNewVorgang)
            {
                ...
                addAttributes(txtAltKennz1);
                addAttributes(txtAltKennz2);
        ...
        private void SelectValues()
        {
            ...
            tmpKennz = objVorerf.Altkenn.Split('-');
            if (tmpKennz.Length == 1)
            {
                if (tmpKennz[0].ToString() != String.Empty) { txtAltKennz1.Text = tmpKennz[0].ToString(); }
            }
            else if (tmpKennz.Length == 2)
            {
                if (tmpKennz[0].ToString() != String.Empty) { txtAltKennz1.Text = tmpKennz[0].ToString(); }
                if (tmpKennz[1].ToString() == String.Empty) { addAttributes(txtAltKennz2); } else { txtAltKennz2.Text = tmpKennz[1].ToString(); }
            }
            ...
            {
                DataRow[] kennzRow = objCommon.tblKennzGroesse.Select("Groesse ='" + objVorerf.KennzForm + "' AND Matnr= '596'");
        ...
        protected void cmdSave_Click(object sender, EventArgs e)
        {
            ...
            if (lblError.Text.Length == 0)
            {
                ...
                RemoveDefault = "";
                if (txtKennz2.Text.Replace("XX9999", "").Length > 0) { RemoveDefault = txtKennz2.Text.ToUpper().Replace(" ", String.Empty); }
                ...
                objVorerf.Vorfuehr = "";
                RemoveDefault = "";
                String RemoveDefault2 = "";
                if (txtAltKennz1.Text.Replace("CK", "").Length > 0) { RemoveDefault = txtAltKennz1.Text.ToUpper(); }
                if (txtAltKennz2.Text.Replace("XX9999", "").Length > 0) { RemoveDefault2 = txtAltKennz2.Text.ToUpper(); }
                objVorerf.Altkenn = RemoveDefault + "-" + RemoveDefault2;
        ...
        private void ClearForm()
        {
            ...
            if (!chkHoldData.Checked)
            {
                ...
                txtAltKennz1.Text = "CK";
                txtAltKennz2.Text = "XX9999";
                chkKennzSonder.Checked = false;
                addAttributes(txtAltKennz1);
                addAttributes(txtAltKennz2);
                addAttributes(txtNrReserviert);
            ...
            else
            {
                ...
                if (drow.Length > 0)
                {
                    ...
                    if (objVorerf.Altkenn.Length > 0) { removeAttributes(txtAltKennz1); removeAttributes(txtAltKennz2); }
