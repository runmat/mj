...
Partial Public Class InfocenterAdmin
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        fileSourcePath = CType(ConfigurationManager.AppSettings("DownloadPathInfocenter"), String) & m_User.KUNNR & "\"
        Literal1.Text = ""
    ...
    Private Sub FillGrouplist()
        Dim listGroups As Security.Groups
        ...
        Using cn As New SqlClient.SqlConnection(m_User.App.Connectionstring)
            With cn
                .Open()
                listGroups = New Security.Groups(m_User, m_User.Customer.CustomerId, cn)
                .Close()
            End With
        End Using

        If listGroups Is Nothing Then
            Exit Sub
        End If

        For Each gr As Security.Group In listGroups
            lbxDocumentGroups.Items.Add(New ListItem(gr.GroupName, gr.GroupId))
        Next
            ...
            Case "showDocument"
                ...
                If lButton IsNot Nothing Then
                    ...
                    If File.Exists(sPfad) Then
                        ...
                        Literal1.Text &= "                          window.open(""DownloadFile.aspx?AppID=" & Session("AppID").ToString & """, ""_blank"", ""left=0,top=0,resizable=YES,scrollbars=YES"");" & vbCrLf
                        Literal1.Text &= "						  //-->" & vbCrLf
