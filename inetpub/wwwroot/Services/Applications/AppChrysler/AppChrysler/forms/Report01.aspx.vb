...
#Region "Methods"
    ...
    Private Sub DoSubmit()
        ...
        Try
            ...
            If txtFahrgestellnummer.Text.Length > 0 OrElse txtKennzeichen.Text.Length > 0 Then
            ...
            Else
                If checkDate() Then
                    ...
                    If checkDateInputFields(txtDatumVon.Text, txtDatumBis.Text) = False Then Exit Sub

                    If checkDateInputFields(txtDatumVonAus.Text, txtDatumBisAus.Text) = False Then Exit Sub

                    If checkDateInputFields(txtSchluesseleingangVon.Text, txtSchluesseleingangBis.Text) = False Then Exit Sub

                    If checkDateInputFields(txtSchluesselversandVon.Text, txtSchluesselversandBis.Text) = False Then Exit Sub

                    objBestand.DatumVon = txtDatumVon.Text
                    objBestand.DatumBis = txtDatumBis.Text
                    objBestand.DatumVonAus = txtDatumVonAus.Text
                    objBestand.DatumBisAus = txtDatumBisAus.Text
                    objBestand.DatumSchluesselerfVon = txtSchluesseleingangVon.Text
                    objBestand.DatumSchluesselerfBis = txtSchluesseleingangBis.Text
                    objBestand.DatumSchluesselversandVon = txtSchluesselversandVon.Text
                    objBestand.DatumSchluesselversandBis = txtSchluesselversandBis.Text
    ...
    Private Function CheckIsEmpty() As Boolean
        ...
        With col
            ...
            .Add(txtDatumVon)
            .Add(txtDatumVonAus)
            .Add(txtSchluesseleingangVon)
            .Add(txtSchluesseleingangBis)
            .Add(txtSchluesselversandVon)
            .Add(txtSchluesselversandBis)
    ...
    Private Function checkDateInputFields(ByVal Von As String, ByVal Bis As String) As Boolean

        If Len(Von & Bis) = 0 Then
            Return True
        End If

        If Von.Length = 0 And Bis.Length > 0 OrElse Von.Length > 0 And Bis.Length = 0 Then
            lblError.Text = "Bitte geben Sie nur Zeiträume ""von/bis"" an!"
            Return False
        Else
            If CDate(Von) > CDate(Bis) Then

                lblError.Text = "Das ""Von-Datum"" darf nicht größer als das ""Bis-Datum"" sein!"
                Return False
            Else
                Return True
            End If
