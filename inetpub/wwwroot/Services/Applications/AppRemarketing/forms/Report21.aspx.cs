...
namespace AppRemarketing.forms
{
    public partial class Report21 : System.Web.UI.Page
    {
        ...
        private Haltedauer m_Haltedauer;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                m_Haltedauer = Session["Haltedauer"] as Haltedauer ?? new Haltedauer();
                Session["Haltedauer"] = m_Haltedauer;
            }
            catch (Exception ex)
            {
        ...
        protected void lbCreate_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                m_Haltedauer.Search(m_User, m_App, this, txtFin.Text, txtKennzeichen.Text, txtInventarnr.Text, ddlVermieter.SelectedValue, von.Value, bis.Value);
            }
            catch (Exception ex)
            {
                return;
        ...
        {
            if ((m_Haltedauer.Result == null) || (m_Haltedauer.Result.Rows.Count == 0))
            {
                SearchMode();
        ...
        {
            if (m_Haltedauer.Result != null)
            {
                rgGrid1.DataSource = m_Haltedauer.Result.DefaultView;
            }
            else
            {
        ...
        protected void rgGrid1_ItemCommand(object sender, GridCommandEventArgs e)
        {
            switch (e.CommandName)
            {
                case RadGrid.ExportToExcelCommandName:
                    ...
                    eSettings.FileName = string.Format("Haltedauer_{0:yyyyMMdd}", DateTime.Now);
