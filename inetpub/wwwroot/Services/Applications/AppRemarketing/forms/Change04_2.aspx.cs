...
{
    public partial class Change04_2 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            lblError.Text = "Bitte wählen Sie Fahrzeuge zur Stornierung aus!";
        ...
        protected void rgGrid2_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            ...
            {
                rgGrid2.DataSource = m_Report.tblError.DefaultView;
            }
            else
            {
        ...
        {
            Response.Redirect("Change04.aspx?AppID=" + (string)Session["AppID"]);
        }

        protected void Storno_Click(object sender, EventArgs e)
        {
            if (CheckGrid() > 0)
            {
                m_Report.Change((string)Session["AppID"], (string)Session.SessionID, this);
                if (m_Report.Status != 0 )
                {
                    lblError.Text = m_Report.Message;
                    if (m_Report.tblError.Rows.Count > 0)
                ...
                else
                {
                    ...
                    for (Int32 i = m_Report.tblAnforderungen.Rows.Count - 1; i >= 0; i--)
                    {
                        ...
                        {
                            m_Report.tblAnforderungen.Rows[i]["Select"]= "-1";
                            m_Report.tblAnforderungen.Rows[i]["Status"]= "Storniert";
                        }
                    }
                    rgGrid1.Rebind();
                    rgGrid1.MasterTableView.GetColumn("Status").Visible = true;
                }
            }
        }
        ...
        protected void rgGrid1_ItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item is GridDataItem)
            {
                ...
                string strMatnr = item["MATNR"].Text;
        ...
        protected void rgGrid1_ItemCommand(object sender, GridCommandEventArgs e)
        {
            switch (e.CommandName)
            {
                case RadGrid.ExportToExcelCommandName:
                    ...
                    eSettings.FileName = string.Format("StornierungVersandauftraege_{0:yyyyMMdd}", DateTime.Now);
