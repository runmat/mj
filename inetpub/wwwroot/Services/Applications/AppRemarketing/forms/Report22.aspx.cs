...
{
    public partial class Report22 : Page
    {
        private User m_User;
        private App m_App;
        private bool isExcelExportConfigured;
        private Widerspruch m_WS;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            m_WS = Common.GetOrCreateObject("WS", () => new Widerspruch(m_User, m_App, (string)Session["AppID"]));
        }
        ...
        {
            m_WS.LoadData(this, ddlVermieter.SelectedValue, txtDatumVon.Text, txtDatumBis.Text, txtVertragsjahr.Text);
            Fillgrid();
        }

        private void Fillgrid()
        {
            if (m_WS.Result.Rows.Count == 0)
            {
                SearchMode();
        ...
        {
            var verm = new Vermieter(m_User, m_App, (string)Session["AppID"]);
            verm.LoadData(this);

            if (verm.Status != 0)
            {
                lblError.Text += verm.Message;
                ddlVermieter.DataSource = null;
            }
            else
            {
                var vermView = verm.Result.DefaultView;
                vermView.Sort = "POS_TEXT";
                ddlVermieter.DataSource = vermView;
                ddlVermieter.DataBind();
            }
        }
        ...
        protected void rgGrid1_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            if (m_WS.Result != null)
            {
                rgGrid1.DataSource = m_WS.Result.DefaultView;
            }
            else
            {
        ...
        protected void rgGrid1_ItemCommand(object sender, GridCommandEventArgs e)
        {
            switch (e.CommandName)
            {
                case RadGrid.ExportToExcelCommandName:
                    ...
                    eSettings.FileName = string.Format("Widersprueche_{0:yyyyMMdd}", DateTime.Now);
