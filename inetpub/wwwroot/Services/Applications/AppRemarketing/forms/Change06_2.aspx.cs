...
{
    public partial class Change06_2 : System.Web.UI.Page
    {
        ...
        protected void rgGrid1_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            if (mObjVersandauftraege.Fahrzeuge != null)
            {
                rgGrid1.DataSource = mObjVersandauftraege.Fahrzeuge.DefaultView;
            }
            else
            {
        ...
        private int CheckGrid()
        {
            ...
            foreach (GridDataItem gridRow in rgGrid1.Items)
            {
                ...
                tmpRow = mObjVersandauftraege.Fahrzeuge.Select("CHASSIS_NUM = '" + gridRow["CHASSIS_NUM"].Text + "'")[0];
                tmpRow.BeginEdit();
                ...
                tmpRow.EndEdit();
                mObjVersandauftraege.Fahrzeuge.AcceptChanges();
            }
        ...
        {
            Response.Redirect("Change06.aspx?AppID=" + (string)Session["AppID"]);
        }
        ...
        protected void cmdWeiter_Click(object sender, EventArgs e)
        {
            if (CheckGrid() > 0)
            {
                Response.Redirect("Change06_3.aspx?AppID=" + (string)Session["AppID"]);
            }
            else
            {
        ...
        {
            CheckBox cbxall = sender as CheckBox;
            ...
            foreach (GridDataItem zeile in rgGrid1.Items)
            {
                ...
                {
                    CheckBox cbx = (CheckBox)zeile.FindControl("chkAuswahl");
