    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If IsPostBack = False Then
            If objHaendler Is Nothing Then
                If Session.Item("AppHaendler07") Is Nothing Then
                    objHaendler = New TempZuEndg(m_User, m_App, Session("AppID").ToString, Session.SessionID.ToString, "")
                    Session.Add("AppHaendler07", objHaendler)
                Else
                    If TypeOf Session.Item("AppHaendler07") Is TempZuEndg Then
                        objHaendler = CType(Session("AppHaendler07"), TempZuEndg)
                    Else
                        objHaendler = New TempZuEndg(m_User, m_App, Session("AppID").ToString, Session.SessionID.ToString, "")
                        Session("AppHaendler07") = objHaendler
                    End If
                End If
            End If
...
#End Region
    ...
    Private Sub DoSubmit()
        ...
        objHaendler.Vertragsnummer = txt_LIZNR.Text
        objHaendler.Fahrgestellnummer = Replace(txtFahrgestellNr.Text, "%", "*")
        objHaendler.KUNNR = m_User.KUNNR
        If rb_Bank.Checked Then
            objHaendler.AnBank = True
        Else
            objHaendler.AnBank = False
        End If


        objHaendler.FILL(Session("AppID").ToString, Session.SessionID, Me)
        ...
        Else
            ...
            Else
                ...
                Response.Redirect("Change07_2.aspx?AppID=" & Session("AppID").ToString, False)
            End If
        End If
    End Sub
