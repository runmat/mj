...
#Region "Methods"
    ...
    Private Sub DoSubmit()
        ...
        If lblHaendlerDetailsNR.Text.Length = 0 Then
            ...
            lblError.Text = "Bitte wählen Sie einen Händler aus."

        Else
            lbSelektionZurueckSetzen.Visible = True

            Dim strFileName As String = Format(Now, "yyyyMMdd_HHmmss_") & m_User.UserName & ".xls"
            m_change = New AppF2BankBaseCredit(m_User, m_App, Session("AppID").ToString, Session.SessionID.ToString, strFileName)
            m_change.AppID = Session("AppID").ToString
            m_change.CreditControlArea = "ZDAD"
            m_change.Customer = lblHaendlerDetailsNR.Text
            ...
            If IsNothing(m_change.Kontingente) OrElse m_change.Kontingente.Rows.Count = 0 Then
                lblError.Text = "Keine Daten zur Anzeige gefunden."
            Else
                Session("SelectedDealer") = lblHaendlerDetailsNR.Text
                Response.Redirect("Change06_2.aspx?AppID=" & Session("AppID").ToString & strTemp)
            End If
