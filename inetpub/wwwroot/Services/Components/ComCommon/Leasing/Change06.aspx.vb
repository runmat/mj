...
Partial Public Class Change06
    ...
    Protected Sub bEingangClick(sender As Object, e As EventArgs) Handles bEingang.Click
        Dim c As New CarportBeauftragung(Me.m_User, Me.m_App, Me)

        If Selected() = False Then Exit Sub

        c.Eingang(Me.GetSelected())

        Me.DoSubmit()
    End Sub

    Protected Sub bBereitmeldungClick(sender As Object, e As EventArgs)
        Dim c As New CarportBeauftragung(Me.m_User, Me.m_App, Me)

        If Selected() = False Then Exit Sub

        c.Bereitmeldung(Me.GetSelected())

        Me.DoSubmit()
    End Sub

    Protected Sub bAusgangClick(sender As Object, e As EventArgs)
        Dim c As New CarportBeauftragung(Me.m_User, Me.m_App, Me)

        If Selected() = False Then Exit Sub

        c.Ausgang(Me.GetSelected())

        Me.DoSubmit()
    End Sub

    Private Function Selected() As Boolean

        Dim i As Integer = Me.GetSelected().Count

        If i < 1 Then
            lblErrorSave.Text = "Bitte treffen Sie eine Auswahl." : Return False
        Else
            Return True
        End If
    ...
    Private Sub DoSubmit()
        ...
        Dim table As DataTable = c.GetAufträge(datumVon, datumBis, "", Me.txtFahrgestellnummer.Text, Me.txtKennzeichen.Text, _
                                                          Me.txtStatus.Text, Me.txtLeistungsart.Text)
