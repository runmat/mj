...
#Region " Functions "
        Public Shared Function GetArchivesForYearMatch(ByVal cn As SqlClient.SqlConnection, yearMatchAtEndOfArchiveName As String) As IEnumerable(Of String)

            Dim archivesMatchingList As List(Of String) = New List(Of String)()

            Dim cmdGetCustomer As New SqlClient.SqlCommand("SELECT * FROM Archiv WHERE EasyArchivName like '%" + yearMatchAtEndOfArchiveName + "'", cn)
            Dim dr As SqlClient.SqlDataReader = cmdGetCustomer.ExecuteReader
            Try

                While dr.Read
                    archivesMatchingList.Add(dr("EasyArchivName").ToString)
                End While
                dr.Close()
