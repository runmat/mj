...
#Region " Methods"
    ...
    Public Overloads Sub Show(ByVal page As Page)
        ...
        Try
            ...
            Dim myProxy As DynSapProxyObj = DynSapProxy.getProxy("Z_M_Gesperrt_Auftrag_Porsche", m_objApp, m_objUser, page)

            myProxy.setImportParameter("I_KUNNR", Right("0000000000" & m_objUser.KUNNR, 10))
            myProxy.setImportParameter("I_KUNNR_ZF", m_strFiliale)
            myProxy.setImportParameter("I_WEB_REPORT", "Change04_5.aspx")
            myProxy.callBapi()

            m_tblAuftraege = myProxy.getExportTable("GT_WEB")
            ...
            m_tblAuftraege.Columns("ERZET").MaxLength = 10
    ...
    Public Overloads Sub Change(ByVal page As Page)
        Try
            ...
            Dim myProxy As DynSapProxyObj = DynSapProxy.getProxy("Z_M_Freigeben_Auftrag_Porsche", m_objApp, m_objUser, page)

            myProxy.setImportParameter("I_KUNNR", Right("0000000000" & m_objUser.KUNNR, 10))
            myProxy.setImportParameter("I_EQUNR", m_strEquipment)
            myProxy.setImportParameter("I_ERNAM", Left(m_objUser.UserName, 12))

            myProxy.setImportParameter("I_KKBER", m_strKunde)
            myProxy.setImportParameter("I_KUNNR_ZF", Right("000000000060" & m_strFiliale, 10))
            myProxy.setImportParameter("I_STORNO", m_strStorno)

            myProxy.setImportParameter("I_VBELN", m_strAuftragsNummer)
            myProxy.setImportParameter("I_WEB_REPORT", "Change05_2.aspx")
...
' 
' *****************  Version 6  *****************
' User: Rudolpho     Date: 4.09.09    Time: 16:55
