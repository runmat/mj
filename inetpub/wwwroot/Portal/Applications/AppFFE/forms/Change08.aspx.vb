...
Partial Public Class Change08
        ...
        Try
            ...
            If (Session("objHaendler") Is Nothing) OrElse (Not IsPostBack) Then
                If Not (Request.QueryString("Aut") Is Nothing) AndAlso (Request.QueryString("Aut").Length > 0) Then
                    ...
                    lbl_BriefNr.Enabled = False
                    txtBriefNr.Enabled = False
                    lbl_FahrgestellNr.Enabled = False
                    txtFahrgNr.Enabled = False
                    lbl_OrderNr.Enabled = False
                    txtOrderNr.Enabled = False
                    lbl_Vertragsnr.Enabled = False
        ...
        Else
            ...
            Else
                ...
                Else
                    ...
                    suche.LeseHaendlerSAP(CType(Session("AppID"), String), CType(Session.SessionID, String), org)
                    ...
                    If Not IsDate(txtVersandDatum.Text) Then
                        'Kein Versanddatum!
                        lblError.Text = "Dokument noch nicht versendet. Änderung nicht möglich!"
                        rb_Temp.Enabled = False
                        rb_Endg.Enabled = False
                        rb_DP.Enabled = False
                        Exit Sub
                    ...
                    kkber = objHaendler.Auftraege.Rows(0)("ZZKKBER")    'Kreditkontrollbereich
                    orgReference = suche.Haendler.Table.Rows(0)("FILIALE")    'OrganizationID
                    objHaendler.Filiale = orgReference
                    ...
                    m_App.CheckForPendingAuthorization(CInt(Session("AppID")), orgReference, Right(hnr, 5), txtFahrgNr.Text, m_User.IsTestUser, strInitiator, intAuthorizationID)
                    If Not strInitiator.Length = 0 Then
                        'Fahrzeug wurde schon mal freigegeben und liegt zur Autorisierung vor
                        lblMessage.Text = "Dieser Vorgang wurde schon einmal geändert und liegt bereits zur Autorisierung vor!"
    ...
    Public Function LeseFilialeHändler(ByVal InputFiliale As String) As String
        Dim FilialeRef As String = ""
        Try
            Dim objDistrikt As New FFE_Bank_Distrikt(m_User, m_App, Session("AppID").ToString, Session.SessionID.ToString)
            objDistrikt.Haendler = InputFiliale
            objDistrikt.ShowUserOrg()
            FilialeRef = objDistrikt.sOrgaID

        Catch ex As Exception
            lblError.Text = "Keine Filialen für diesen Kunden <br>(" & ex.Message & ")."
            Return ""
        Finally
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If Not (Request.QueryString("Aut") Is Nothing) AndAlso (Request.QueryString("Aut").Length > 0) Then
            ...
            Else
                ...
                HelpProcedures.fakeLogEntryForAutorisierung(logApp, m_User, Session.SessionID, Session("AppID"), "Änderung Status für " & Right(CType(objHaendler.Auftraege.Rows(0)("PARNR"), String), 5) & " erfolgreich autorisiert.", , "Änderung Status")
        ...
        Else
            ...
            Dim intAuthorizationID As Int32
            org = Right(CType(objHaendler.Auftraege.Rows(0)("PARNR"), String), 5) 'Organisation holen
            orgReference = objHaendler.Filiale  'OrganisationID holen
            m_App.CheckForPendingAuthorization(CInt(Session("AppID")), orgReference, Right(hnr, 5), txtFahrgNr.Text, m_User.IsTestUser, strInitiator, intAuthorizationID)
            If Not strInitiator.Length = 0 Then
                'Fahrzeug wurde schon mal freigegeben und liegt zur Autorisierung vor
                lblMessage.Text = "Dieser Vorgang wurde schon einmal geändert und liegt bereits zur Autorisierung vor!"
            ...
            intAuthorizationID = WriteAuthorization(m_App.Connectionstring, CInt(Session("AppID")), m_User.UserName, orgReference, Right(hnr, 5), txtFahrgNr.Text, "", "", m_User.IsTestUser, DetailArray)
            ...
            If (intAuthorizationID > 0) Then
                ...
                lbl_Status.Enabled = False
                btnSave.Visible = False
                lblMessage.Text = "Vorgang zur Autorisierung gespeichert."
    ...
    Private Sub Linkbutton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Linkbutton1.Click
        Try
            ...
            HelpProcedures.fakeLogEntryForAutorisierung(logApp, m_User, Session.SessionID, Session("AppID"), "Änderung Status für " & objHaendler.Auftraege.Rows(0)("PARNR") & " erfolgreich autorisiert.")
            ...
            txtVertragsNr.Enabled = False
        Catch ex As Exception
            HelpProcedures.fakeLogEntryForAutorisierung(logApp, m_User, Session.SessionID, Session("AppID"), "Fehler bei der Autorisierung einer Änderung Fälligkeit + Status für " & objHaendler.Auftraege.Rows(0)("PARNR") & ", (Fehler: " & ex.Message & ")", "ERR")
    ...
    Protected Sub rb_DP_CheckedChanged(ByVal sender As Object, ByVal e As EventArgs) Handles rb_DP.CheckedChanged
        If rb_DP.Checked Then
            lbl_Faelligkeit.Enabled = True
            txt_Faelligkeit.Enabled = True
        End If
