            ...
            For Each item In GridView1.Rows
                ...
                If chkBox.Checked = True Then
                    ...
                    If FreigabeFlag = "A" Then
                        txtBox = item.FindControl("txtAblehnungsgrund")
                        If txtBox.Text.Trim.Length > 0 Then
                            dRow("Ablehnungsgrund") = txtBox.Text
                            bchecked = True
                        Else
                            item.Cells(6).BackColor = System.Drawing.Color.FromArgb(196, 0, 0)
                    ...
                    For Each tmpRow As DataRow In m_report.Result.Rows
                        If tmpRow("zurFreigabe").ToString = "X" Then
                            ...
                            intAuthorizationID = WriteAuthorization(m_App.Connectionstring, CInt(Session("AppID")), m_User.UserName, m_User.Organization.OrganizationId,
                                                                    tmpRow("Treunehmer").ToString, tmpRow("Fahrgestellnummer").ToString, "Freigabe", "",
                            ...
                            logApp.WriteEntry("APP", m_User.UserName, Session.SessionID, CInt(Session("AppID")),
                                              ...
                                              tmpRow("Treunehmer").ToString, "Freigabe für Treunehmer " & tmpRow("Treunehmer").ToString & " erfolgreich initiiert.",
                ...
                If m_User.Groups.ItemByID(m_User.GroupID).Authorizationright > 0 Then
                    For Each tmpRow As DataRow In m_report.Result.Rows
                        If tmpRow("zurFreigabe").ToString = "X" Then
                            ...
                            m_App.CheckForPendingAuthorization(CInt(Session("AppID")), m_User.Organization.OrganizationId, m_User.KUNNR,
                                                               tmpRow("Fahrgestellnummer").ToString, m_User.IsTestUser, strInitiator, intAuthorizationID)
                            If Not strInitiator.Length = 0 Then
                                'Fahrzeug wurde schon mal freigegeben und liegt zur Autorisierung vor
                                lblError.Text = "Diese Anforderung wurde schon einmal freigegeben und liegt bereits zur Autorisierung vor!"
                                Exit Sub
                            ...
                            intAuthorizationID = WriteAuthorization(m_App.Connectionstring, CInt(Session("AppID")), m_User.UserName,
                                                                    m_User.Organization.OrganizationId, tmpRow("Treunehmer").ToString,
                                                                    tmpRow("Fahrgestellnummer").ToString, "Ablehnung", "", m_User.IsTestUser, DetailArray)
                            tmpRow("Status") = "Aut."
                            logApp.WriteEntry("APP", m_User.UserName, Session.SessionID, CInt(Session("AppID")),
                                              m_User.Applications.Select("AppID = '" & Session("AppID").ToString & "'")(0)("AppFriendlyName").ToString,
                                              tmpRow("Treunehmer").ToString, "Ablehnung für Treunehmer " & tmpRow("Treunehmer").ToString & " erfolgreich initiiert.",
                                              m_User.CustomerName, m_User.Customer.CustomerId, m_User.IsTestUser, 0, logApp.InputDetails)
