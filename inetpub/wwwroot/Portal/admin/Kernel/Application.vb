...
#Region " Constructor "
        ...
        Public Sub New(ByVal intAppID As Integer, _
                       ...
                       ByVal AppSchwellwert As String)
            m_intAppID = intAppID
        ...
        Public Sub ReAssign(ByVal cn As SqlClient.SqlConnection, ByVal intAppID As Integer, ByVal intParentAppID As Integer)
            If intParentAppID > 0 Then
                Try
                    ...
                    da = New SqlClient.SqlDataAdapter("SELECT GroupID,AuthorizationLevel,WithAuthorization FROM Rights WHERE AppID=@AppID", cn)
                    da.SelectCommand.Parameters.AddWithValue("@AppID", intParentAppID)
                    dt = New DataTable()
                    da.Fill(dt)
                    ...
                    If Not dt Is Nothing Then
                        For Each dr In dt.Rows
                            cmd = New SqlClient.SqlCommand()
                            cmd.Connection = cn
                            cmd.CommandText = "INSERT INTO Rights VALUES (@GroupID,@AppID, @AuthorizationLevel,@WithAuthorization)"
                            cmd.Parameters.AddWithValue("@GroupID", CInt(dr("GroupID")))
                            cmd.Parameters.AddWithValue("@AppID", intAppID)
                            cmd.Parameters.AddWithValue("@AuthorizationLevel", CInt(dr("AuthorizationLevel")))
                            cmd.Parameters.AddWithValue("@WithAuthorization", CInt(dr("WithAuthorization")))
                            cmd.ExecuteNonQuery()
                        Next
                    End If
...
' ************************************************
' $History: Application.vb $
' 
' *****************  Version 5  *****************
' User: Rudolpho     Date: 26.08.10   Time: 12:33
...
' 
' *****************  Version 7  *****************
' User: Rudolpho     Date: 9.01.08    Time: 10:39
