Partial Public Class GridNavigation
    ...
    Private Sub DataGrid1_DataBound(ByVal sender As Object, ByVal e As System.EventArgs) Handles mDataGrid.ItemDataBound
        ...
        Dim i As Integer
        Dim LinkTabel As New DataTable

        LinkTabel.Columns.Add("Index", System.Type.GetType("System.String"))
        LinkTabel.Columns.Add("Page", System.Type.GetType("System.String"))
        Dim RepRow As DataRow

        For i = 0 To mDataGrid.PageCount - 1

            ' Create a ListItem object to represent a page.
            Dim pageNumber As Integer = i
            RepRow = LinkTabel.NewRow
            RepRow("Index") = pageNumber
            RepRow("Page") = pageNumber + 1
            LinkTabel.Rows.Add(RepRow)

        Next i

        If LinkTabel.Rows.Count > 0 Then
            Repeater1.DataSource = LinkTabel
            Repeater1.DataBind()

            For i = 0 To LinkTabel.Rows.Count - 1
                If i = mdatagrid.CurrentPageIndex Then
                    Dim lbtn As LinkButton
                    lbtn = CType(Repeater1.Controls(i).Controls(0).FindControl("lbtnPage"), LinkButton)
                    lbtn.CssClass = "active"
                    Exit For
                End If
            Next
    ...
    Private Sub ddlPageSize_SelectedIndexChanged1(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlPageSize.SelectedIndexChanged
        Select Case False
            Case mDataGrid Is Nothing
                mDataGrid.PageSize = CInt(ddlPageSize.SelectedValue)
            Case mGridView Is Nothing
                mGridView.PageSize = CInt(ddlPageSize.SelectedValue)
            Case Else
    ...
    Public Sub lbtnPage_PageIndexChanging(ByVal sender As Object, ByVal e As System.EventArgs)
        RaiseEvent PagerChanged(CInt(CType(sender, LinkButton).CommandArgument))
    End Sub
