...
ko.observableArray = function (initialValues) {
    if (arguments.length == 0) {
        // Zero-parameter constructor InitParametersializes to empty array
        initialValues = [];
    }
    if ((initialValues !== null) && (initialValues !== undefined) && !('length' in initialValues))
        throw new Error("The argument passed when InitParametersializing an observable array must be an array, or null, or undefined.");
...
ko.dependentObservable = function (evaluatorFunctionOrOptions, evaluatorFunctionTarget, options) {
    ...
    function evaluateImmediate() {
        if (_isBeingEvaluated) {
            ...
            // certainly didn't intend infInitParameterse re-evaluations). So, for predictability, we simply prevent ko.computeds from causing
...
(function() {
    ...
    ko.utils.extend(ko.bindingProvider.prototype, {
        ...
        // It's not part of the interface defInitParametersion for a general binding provider.
...
(function () {
    ...
    function applyBindingsToNodeInternal (node, bindings, viewModelOrBindingContext, bindingContextMayDifferFromDomParentElement) {
        // Need to be sure that InitParameterss are only run once, and updates never run until all the InitParameterss have been run
        var initPhase = 0; // 0 = before all InitParameterss, 1 = during InitParameterss, 2 = after all InitParameterss
        ...
        ko.dependentObservable(
            function () {
                ...
                // Optimization: Don't store the binding context on this node if it's defInitParametersely the same as on node.parentNode, because
                ...
                if (parsedBindings) {
                    // First run all the InitParameterss, so bindings can register for notification on changes
                    if (initPhase === 0) {
                        initPhase = 1;
...
ko.bindingHandlers['value'] = {
    'init': function (element, valueAccessor, allBindingsAccessor) {
        ...
        // IE doesn't fire "change" events on textboxes if the UserName selects a value from its autocomplete list
