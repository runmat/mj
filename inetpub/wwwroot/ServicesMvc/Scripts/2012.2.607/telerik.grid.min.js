    ...
    var b = a.telerik; var j = /"+\\\/Date\((.*?)\)\\\/"+/g; var k = "tr:not(.t-grouping-row,.t-group-footer,.t-detail-row,.t-no-data,.t-footer-template):visible", d = ">td:not(.t-group-cell,.t-hierarchy-cell):visible", f = k + d + ":first", g = "t-state-focused"; b.scripts.push("telerik.grid.js"); function m(o) { return new Function("data", ("var p=[];with(data){p.push('" + unescape(o).replace(/[\r\t\n]/g, " ").replace(/'(?=[^#]*#>)/g, "\t").split("'").join("\\'").split("\t").join("'").replace(/<#=(.+?)#>/g, "',$1,'").split("<#").join("');").split("#>").join("p.push('") + "');}return p.join('');")) } function e(o) { return (o != null ? o + "" : "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;") } function i(q, s, r) { var o = a("<col />").css("width", s[r].width), t, u, p = 0; for (t = 0, u = s.length; t < u; t++) { if (t >= r && p) { break } if (!s[t].hidden) { p++ } } if (t > r) { q.eq(p - 1).before(o) } else { q.eq(p - 1).after(o) } } b.grid = function (p, w) {
        ...
        ajaxOptions: function (o) {
            var p = {
                ...
                error: a.proxy(function (s, r) { if (b.ajaxError(this.element, "error", s, r)) { return } }, this),
                complete: a.proxy(this.hideBusy, this),
                success: a.proxy(function (r, t, u) {
                    try { r = eval("(" + r + ")") } catch (s) { if (!b.ajaxError(this.element, "error", u, "parsererror")) { alert("Error! The requested URL did not return JSON.") } return }
                    if (o.commandName) { this._onComplete({ name: o.commandName, response: r }) } r = r.d || r; if (o.hasErrors && o.hasErrors(r)) { if (!b.trigger(this.element, "error", { XMLHttpRequest: u, textStatus: "modelstateerror", modelState: r.modelState })) { o.displayErrors(r) } return } this.dataSource.success(r)
                }, this)
            };
            a.extend(p, o); var q = this.ws ? p.data.state = {} : p.data; if (this._isServerOperation()) { q[this.queryString.page] = this.currentPage; q[this.queryString.size] = this.pageSize; q[this.queryString.groupBy] = this.groupBy; q[this.queryString.filter] = (this.filterBy || "").replace(/\"/g, '\\"') } q[this.queryString.orderBy] = this.orderBy || ""; q[this.queryString.aggregates] = a.map(this.columns, function (r) { if (r.aggregates) { return r.member + "-" + r.aggregates.join("-") } }).join("~"); if (this.ws) { p.data = b.toJson(p.data); p.contentType = "application/json; charset=utf-8" }
            return p
        },
        showBusy: function () {
        ...
        hideBusy: function () {
            //alert('hide');
            LoadingHide(); setTimeout('LoadingHide()', 100); setTimeout('LoadingHide()', 1000); setTimeout('LoadingHide()', 3000);
            clearTimeout(this.busyTimeout);
            a("> .t-grid-pager .t-status .t-icon", this.element).removeClass("t-loading");
        },
        serverRequest: function () { if (this.operationMode === "client") { this.ajaxRequest() } else { location.href = b.formatString(unescape(this.urlFormat), this.currentPage, this.orderBy || "~", this.groupBy || "~", encodeURIComponent(this.filterBy) || "~", this.pageSize || "~") } }, _isServerOperation: function () { return this.operationMode !== "client" },
        ajaxRequest: function (o) {
            try {
                if (getFilteredData_DisableDataBound())
                    return;
            }
            catch (e) { }
            
            var u = this, t = u.pageSize > 0, s = u.pageSize, q = u.currentPage, p = a.map(u.columns, function (v) { return a.map(v.aggregates || [], function (w) { return { field: v.member, aggregate: w} }) });
            if (q > 1 && u.pageOnScroll && !u._pagingInProgress) { s = q * u.pageSize; q = 1 } var r = { page: q, sortedColumns: u.sorted, filteredColumns: a.grep(u.columns, function (v) { return v.filters }) };
            if (b.trigger(u.element, "dataBinding", r)) { return }
            if (!u.ajax && !u.ws && this.operationMode !== "client") { return }
            if (u.dataSource.transport.options && u.dataSource.transport.options.read) {
                u.dataSource.transport.options.read.url = this.url("selectUrl")
            }
            if (u._isServerOperation()) { u.showBusy() } u.dataSource.query(a.extend({ page: q, pageSize: t ? s : n, sort: a.map(u.sorted, function (v) { return { field: v.member, dir: v.order} }), filter: a.map(a.grep(u.columns, function (v) { return v.filters }), function (v) { return u._translateFilterExpr(v, v.filters || []) }), group: a.map(u.groups, function (v) { return { field: v.member, dir: v.order, aggregates: p} }), aggregates: p }, a.extend({}, r.data, o)))
        },
        _translateFilterExpr: function (o, p) {
            var q = this;
            return a.map(p, function (s) {
                if (s.filters) {
                    return { logic: s.logic, filters: q._translateFilterExpr(o, s.filters) }
                } else {
                    var u = s.value;
                    if (o.type == "Number") {
                        u = parseFloat(u)
                        ...
                        if (o.type == "Date") {
                            if (typeof u === "string") {
                                var r = /^\/Date\((.*?)\)\/$/.exec(u);
                                if (r) {
                                    u = new Date(parseInt(r[1]))
                                } else {
                                    var t = o.format ? /\{0(:([^\}]+))?\}/.exec(o.format)[2] : b.cultureInfo.shortDate; u = b.datetime.parse({ value: u, format: t }).toDate()
                                }
                            }
                        }
                    }
                    return { field: o.member, operator: s.operator, value: u }
                }
            })
        },
        valueFor: function (o) {
